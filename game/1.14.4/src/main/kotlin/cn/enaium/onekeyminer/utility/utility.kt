...
fun World.getBlockName(blockPos: BlockPos): String {
    return this.getBlockState(blockPos).block.dropTableId.getName()
}
...
fun PlayerEntity.toCommon(world: World): Player {
    return object : Player {
        override val host: Boolean
            get() = this@toCommon.uuid.equals(MinecraftClient.getInstance().session.uuid)
        ...
        override fun canMine(pos: cn.enaium.onekeyminer.common.BlockPos): Boolean {
            ...
            return this@toCommon.inventory.getInvStack(this@toCommon.inventory.selectedSlot)?.item?.canMine(
        ...
        override val direction: Direction
            get() = Direction.valueOf(this@toCommon.horizontalFacing.name)
    }
}