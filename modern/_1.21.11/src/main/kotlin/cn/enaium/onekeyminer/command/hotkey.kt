        ...
        ROOT.then(
            CommandManager.literal("hotkey").executes { context: CommandContext<ServerCommandSource> ->
                context.source.sendFeedback(
                    { Text.translatable("command.hotkey.get", model.hotkey) }, false
                )
                Command.SINGLE_SUCCESS
            }.then(
                CommandManager.argument("hotkey", BoolArgumentType.bool())
                    .executes { context: CommandContext<ServerCommandSource> ->
                        model.hotkey = BoolArgumentType.getBool(context, "hotkey")
                        context.source.sendFeedback(
                            { Text.translatable("command.hotkey.success", model.hotkey) },
                            false
                        )
                        Command.SINGLE_SUCCESS
                    })
