...
public class LimitCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(ROOT.then(CommandManager.literal("limit").executes(context -> {
            context.getSource().sendFeedback(Text.translatable("command.limit.get", Config.getModel().limit), false);
            ...
            if (limit <= 0) {
                context.getSource().sendError(Text.translatable("command.limit.error"));
                return Command.SINGLE_SUCCESS;
            }
            Config.getModel().limit = limit;

            context.getSource().sendFeedback(Text.translatable("command.limit.success", limit), false);
            return Command.SINGLE_SUCCESS;
        }))));
    }
}
