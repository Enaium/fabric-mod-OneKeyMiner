        ...
        ROOT.then(
            CommandManager.literal("limit").executes { context: CommandContext<ServerCommandSource> ->
                context.source.sendFeedback(
                    TranslatableText("command.limit.get", model.limit), false
                )
                ...
                CommandManager.argument("limit", IntegerArgumentType.integer())
                    .executes { context: CommandContext<ServerCommandSource> ->
                        ...
                        if (limit <= 0) {
                            context.source.sendError(TranslatableText("command.limit.error"))
                            return@executes Command.SINGLE_SUCCESS
                        }
                        model.limit = limit
                        context.source.sendFeedback(TranslatableText("command.limit.success", limit), false)
                        Command.SINGLE_SUCCESS
                    })
        )
    )
