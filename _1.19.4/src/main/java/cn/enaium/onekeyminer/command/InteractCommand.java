...
public class InteractCommand {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
        dispatcher.register(ROOT.then(CommandManager.literal("interact").executes(context -> {
            context.getSource().sendFeedback(Text.translatable("command.interact.get", Config.getModel().interact), false);
            ...
            context.getSource().sendFeedback(Text.translatable("command.interact.success", Config.getModel().interact), false);
            return Command.SINGLE_SUCCESS;
        }))));
    }
}
