...
public class ToolSelectScreen extends Screen {
    ...
    public void render(int mouseX, int mouseY, float delta) {
        ...
        addButton(new ButtonWidget(0, 150, 200, 20, new TranslatableText("button.shears").asString(), e -> {
            MinecraftClient.getInstance().openScreen(new BlockListScreen(Config.getModel().shears));
        }));

        addButton(new ButtonWidget(0, 150, 200, 20, new TranslatableText("button.any").asString(), e -> {
            MinecraftClient.getInstance().openScreen(new BlockListScreen(Config.getModel().any));
        }));
