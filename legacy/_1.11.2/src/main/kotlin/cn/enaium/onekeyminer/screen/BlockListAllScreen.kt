    ...
    fun get(): List<BlockListWidget.Entry> {
        ...
        for (block in Block.REGISTRY) {
            if (textFieldWidget!!.text != "") {
                if (getName(Block.REGISTRY.getIdentifier(block)).contains(textFieldWidget!!.text) || TranslatableText(
                        block.translationKey
                    ).asFormattedString().contains(textFieldWidget!!.text)
                ) {
                    temp.add(BlockListWidget.Entry(getName(Block.REGISTRY.getIdentifier(block))))
                }
            } else {
                temp.add(BlockListWidget.Entry(getName(Block.REGISTRY.getIdentifier(block))))
            }
